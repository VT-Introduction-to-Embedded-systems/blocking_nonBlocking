...
int main(void)
{
    ...
    printList_blocking(&g_sContext, 6);

    // These numbers are randomly chose to show you printing 4 different lists back to back in the while(1) loop w/o blocking
    int printList[4] = {7, 3, 1, 9};
    int index  = 0;

    while (1)
    {
        // When printing one list is over, it starts the next list
        if (printList_nonBlocking(&g_sContext, printList[index]))
            index++;
        if (index==4)
            index = 0;
